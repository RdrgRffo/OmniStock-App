...
public class EncryptionService {
     ...
     * Deriva una clave AES de 256 bits (32 bytes) usando SHA-256,
     * permitiendo cualquier longitud en el secreto configurado.
     */
    private SecretKeySpec getKeyFromSecret() {
        try {
            MessageDigest sha = MessageDigest.getInstance("SHA-256");
            byte[] keyBytes = sha.digest(encryptionSecret.getBytes(StandardCharsets.UTF_8));
            // Tomamos solo los primeros 32 bytes (256 bits) para AES-256
            keyBytes = Arrays.copyOf(keyBytes, 32);
            return new SecretKeySpec(keyBytes, ALGORITHM);
        } catch (Exception e) {
            throw new RuntimeException("Error al derivar la clave AES desde el secreto", e);
        }
    }
