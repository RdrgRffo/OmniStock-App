...
class ExportServiceTest {
    ...
    @BeforeEach
    void setUp() {
        exportService = new ExportService(analyticsService);
        mockAnalyticsEmptyResponses();
    }

    private void mockAnalyticsEmptyResponses() {
        when(analyticsService.getPriceIndexVariation(anyInt(), anyDouble())).thenReturn(List.of());
        when(analyticsService.getStockoutRates()).thenReturn(List.of());
        when(analyticsService.getPriceDispersion(anyInt())).thenReturn(List.of());
        when(analyticsService.getMoqDistribution()).thenReturn(List.of());
        when(analyticsService.getConditionMix()).thenReturn(List.of());
        when(analyticsService.getCostCoverage(anyInt())).thenReturn(List.of());
        when(analyticsService.getStockVolatility(anyInt(), anyInt())).thenReturn(List.of());
        when(analyticsService.getCatalogGrowth(anyInt())).thenReturn(List.of());
        when(analyticsService.getPriceStability(anyInt(), anyInt())).thenReturn(List.of());
        when(analyticsService.getTradingOpportunities(anyInt())).thenReturn(List.of());
    }
    ...
    @Nested
    @DisplayName("generateDashboardExcel")
    class GenerateDashboardExcel {

        @Test
        @DisplayName("Debe generar XLSX con todas las hojas esperadas (19 hojas)")
        void shouldGenerateXlsxWithAllSheets() throws Exception {
            DashboardSummaryDto summary = createSampleSummary();
            mockAnalyticsEmptyResponses();
            byte[] xlsxBytes = exportService.generateDashboardExcel(summary);
