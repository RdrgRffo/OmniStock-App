...
class UserServiceTest {
    ...
    @BeforeEach
    void setUp() {
        userService = new UserService(userRepository, roleRepository, passwordEncoder, roleService, budgetRepository);
    }
    ...
    class DeleteById {
        ...
        @Test
        @DisplayName("Debe desactivar usuario (soft delete) por ID")
        void shouldSoftDeleteUser() {
            User user = createUser(1, "testuser", "test@test.com", "Test User", "ROLE_USER");
            when(userRepository.findById(1)).thenReturn(Optional.of(user));
            when(budgetRepository.countByCreatedByAndStatus("testuser", "DRAFT")).thenReturn(0L);
            when(budgetRepository.countByCreatedByAndStatus("testuser", "FINALIZED")).thenReturn(0L);
