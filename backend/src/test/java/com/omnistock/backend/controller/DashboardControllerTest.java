...
class DashboardControllerTest {
    ...
    @Nested
    @DisplayName("GET /api/v1/dashboard/export/excel")
    class ExportExcel {
        ...
        void shouldExportExcel() throws Exception {
            ...
            when(dashboardService.getDashboardSummary()).thenReturn(summary);
            when(exportService.generateDashboardExcel(summary)).thenReturn(new byte[]{0x50, 0x4B, 0x03, 0x04});

            mockMvc.perform(get("/api/v1/dashboard/export/excel"))
                    .andExpect(status().isOk())
