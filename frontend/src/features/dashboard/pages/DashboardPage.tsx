...
import { Building2, Download, FileText, Gauge, LayoutDashboard, Package, ShieldCheck, TriangleAlert } from 'lucide-react';
...
const PIE_COLORS = ['#f97316', '#2563eb', '#16a34a', '#dc2626', '#ca8a04'];
...
const BrandPieChart = ({ data }: { data: ChartDataDto[] }) => {
  return (
    <div style={{ height: '300px', width: '100%' }}>
      <ResponsiveContainer width="100%" height="100%">
        <PieChart>
          <Pie data={data} dataKey="value" nameKey="label" cx="50%" cy="50%" innerRadius={60} outerRadius={80} fill="#f97316" paddingAngle={5}>
            {data.map((_, index) => <Cell key={`cell-${index}`} fill={PIE_COLORS[index % PIE_COLORS.length]} />)}
          </Pie>
          <Tooltip />
          <Legend formatter={(value: string) => <span style={{ color: '#000' }}>{value}</span>} />
        </PieChart>
      </ResponsiveContainer>
    </div>
  );
  ...
  <div style={{ height: '300px', width: '100%' }}>
    <ResponsiveContainer width="100%" height="100%">
      <BarChart data={data} layout="vertical" margin={{ top: 5, right: 20, left: 40, bottom: 5 }}>
        ...
        <Tooltip />
        <Legend />
        <Bar dataKey="value" fill="#f97316" name="Stock" />
      </BarChart>
    </ResponsiveContainer>
  </div>
);
...
const SlaGradeBadge = ({ grade }: { grade: string }) => {
  const colors: Record<string, string> = { A: '#166534', B: '#a16207', C: '#9a3412', D: '#991b1b' };
...
const DashboardPage = () => {
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      link.href = url;
      link.setAttribute('download', `dashboard_data_${new Date().toISOString().split('T')[0]}.xlsx`);
