...
const translateTransformation = (type: string): string => {
  const translations: Record<string, string> = {
    'DIRECT': 'Directo',
    'NESTED': 'Anidado',
    'FIND_IN_ARRAY': 'Buscar en Array',
    'SPLIT': 'Dividir',
  };
              ...
              <ul style={{ margin: 0, paddingLeft: '1.1rem' }}>
                ...
                <li>
                  ...
                  <ul style={{ marginTop: '0.25rem', paddingLeft: '1.1rem' }}>
                    <li><strong>Plano (Directo)</strong>: una clave simple (ej.: <code>price</code>, <code>stock_level</code>).</li>
                    <li><strong>Jerárquico (Anidado)</strong>: usando notación de puntos (ej.: <code>commercial.price_data.amount</code>).</li>
                    <li><strong>Array clave-valor (Buscar en Array)</strong>: patrón <code>attributes[key=socket]</code>, donde se busca en el array el elemento cuya propiedad <code>key</code> sea <code>socket</code> y se toma su <code>val</code>.</li>
                    <li><strong>Texto con posiciones (Dividir)</strong>: patrón <code>feat[0]</code>, <code>feat[1]</code>... para extraer la posición 0, 1, etc. de una lista separada por <code>;</code> (ej.: <code>feat = "E-ATX;DDR5;USB4"</code>).</li>
                  </ul>
                </li>
                <li>
                  <strong>Transformation</strong>: selecciona cómo debe leer el backend el campo externo. Solo se admiten:
                  <code style={{ marginLeft: '0.25rem' }}>Directo (DIRECT)</code>, <code>Anidado (NESTED)</code>, <code>Buscar en Array (FIND_IN_ARRAY)</code> y <code>Dividir (SPLIT)</code>.
                </li>
              </ul>
                ...
                <tbody>
                  {mappings?.map((mapping) => (
                    <tr key={mapping.mappingId}>
                      ...
                      <td>{translateTransformation(mapping.transformationType)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
