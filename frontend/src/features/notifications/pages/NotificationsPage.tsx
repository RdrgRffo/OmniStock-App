...
type ActiveTab = NotificationTab | 'todas';

const ALL_TABS: NotificationTab[] = ['general', 'errores', 'ofertas', 'nuevo-producto'];

const TAB_DEFINITIONS: Array<{ key: ActiveTab; label: string; icon: React.ReactNode }> = [
  { key: 'todas', label: 'Todas', icon: <Bell size={16} /> },
  { key: 'general', label: 'General', icon: <Info size={16} /> },
...
const NotificationsPage = () => {
  ...
  useEffect(() => {
    if (activeTab === 'todas') {
      ...
      ALL_TABS.forEach((tab) => void loadTab(tab));
    } else {
  ...
  const activeList = useMemo(() => {
    if (activeTab === 'todas') {
      ...
      const all = ALL_TABS.flatMap((tab) => notificationsByTab[tab] ?? []);
      return all.sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime());
    }
    return notificationsByTab[activeTab] ?? [];
  }, [activeTab, notificationsByTab]);

  const totalCount = useMemo(() => {
    return counts.general + counts.errores + counts.ofertas + counts.nuevoProducto;
  }, [counts]);
    ...
    <div className="notifications-page">
      <header className="notifications-header">
        ...
        <div className="notifications-actions">
          ...
          {activeList.length > 0 && (
            <button
              className="btn btn-secondary"
              onClick={() => void markAllAsRead(activeTab === 'todas' ? 'general' : activeTab)}
            >
      ...
      <div className="notifications-tabs" role="tablist" aria-label="Categorías de notificaciones">
        {TAB_DEFINITIONS.map((tab) => {
          const count = tab.key === 'todas' ? totalCount : getTabCount(tab.key, counts);
          return (
            <button
              key={tab.key}
              type="button"
